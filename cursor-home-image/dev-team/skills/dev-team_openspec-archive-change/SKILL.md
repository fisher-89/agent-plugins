---
name: dev-team_openspec-archive-change
...
1. **Resolve the change and check completion status**
   ...
   Call `mcp__user-dev-team_mcp__change_list` once to get all active changes (not archived).
...
- Use `mcp__user-dev-team_mcp__change_list` (`artifacts` + `workflow_done`) for completion checking
