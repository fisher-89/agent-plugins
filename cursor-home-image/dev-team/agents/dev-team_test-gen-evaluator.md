...
Call `mcp__user-dev-team_mcp__phase_log` with `phase: "test-gen"` to write the evaluation result. Other parameter types are defined by the tool schema; verdict is auto-calculated from checklist (all pass → pass).
