---
name: dev-team_workflow-test-only
description: |
...
Evaluator agent 内部调用 `mcp__user-dev-team_mcp__phase_log` 将 verdict/report/checklist 写入 `workflow.json`（`eval` 字段）。Skill 通过 `phase_next` 的 `last_result` 字段读取 verdict。
...
Call `mcp__user-dev-team_mcp__change_list()` to get active changes.
...
```
mcp__user-dev-team_mcp__change_create({ name: "<change-name>", workflow_type: "test-only" })
```
...
`workflow.json.eval` through `mcp__user-dev-team_mcp__phase_log` — never by writing the file directly.
...
LOOP:
  -- Phase Check --
  gate = mcp__user-dev-team_mcp__phase_next({change: "<change-name>", run_id: "<run_id>"})
  ...
  if gate.last_result.verdict == "fail":
    ...
    if report mentions code bugs:
      ...
      `AskQuestion`: continue workflow or terminate
  ...
  mcp__user-dev-team_mcp__phase_start({ change: "<change-name>", phase: gate.next_phase })
