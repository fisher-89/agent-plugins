...
```
mcp__user-dev-team_mcp__test_detect_frameworks({files:[<test files>]})
```
...
```
node "__INSTALL_PLUGIN_ROOT__/bin/dev-team_cli.cjs" run_static_analysis
```
...
- Use the tool `mcp__user-dev-team_mcp__test_detect_frameworks` to detect the project test framework(s)
