...
- The change scope comes from the design/proposal declaration × the change file inventory fetched via `mcp__plugin_dev-team_dev-team__workflow_files` × the filesystem
...
3. Call `mcp__plugin_dev-team_dev-team__workflow_files({ change: "<change-name>" })` 获取 `files` 清单圈定审查范围；run `git diff` to view the modification content
