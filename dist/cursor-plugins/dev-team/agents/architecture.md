...
1. Call `mcp__plugin_dev-team_dev-team__archi_check` on the active change's file inventory (the recorded `files.written` in `workflow.json`):

   ```
   mcp__plugin_dev-team_dev-team__archi_check({change: "<change-name>"})
   ```
...
1. **Create**: Gather background, decision, consequences, alternatives, and scope from the user. Then call:
   ...
   ```
   mcp__plugin_dev-team_dev-team__archi_decide({
