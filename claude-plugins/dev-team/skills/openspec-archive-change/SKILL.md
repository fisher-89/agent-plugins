...
1. **Resolve the change and check completion status**
   ...
   **If no change name was provided** — use `AskUserQuestion` to let the user
   ...
   **If any required artifacts are missing or `workflow_done` is `false`:**
   ...
   - If the change lacks `workflow.json`, or the file is malformed, `workflow_done` is
     ...
     tell them it must be created/fixed via `mcp__plugin_dev-team_dev-team__change_create` (or written by the
