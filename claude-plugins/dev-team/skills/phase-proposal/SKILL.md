...
Use `AskUserQuestion` to confirm the workflow type (default `requirement`):
...
If response `modified` is true, recall `mcp__plugin_dev-team_dev-team__phase_next(change=<name>, run_id=<run_id>)`, continue to `### Explore handoff`.
