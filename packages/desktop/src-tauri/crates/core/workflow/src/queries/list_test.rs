...
const V1_WORKFLOW: &str = r#"{ "workflow_type": "requirement", "created": "2026-01-01", "eval": [] }"#;
...
fn active与archive全量返回_代际标注与磁盘事实一致() {
    ...
    assert_eq!(by_name("active-v2").source, crate::queries::ChangeSource::Active);
    ...
    assert_eq!(
        list.archive_groups[0].changes[0].inventory,
        Inventory::V0
    );
    assert_eq!(
        list.archive_groups[1].changes[0].inventory,
        Inventory::V1
    );
...
#[test]
fn archive按月前缀折叠_组间新月份在前_组内按名稳定() {
    ...
    let months: Vec<Option<&str>> = list
        ...
        .collect();
    assert_eq!(months, vec![Some("2026-03"), Some("2026-01"), Some("2025-12")]);
    ...
    let jan: Vec<&str> = list.archive_groups[1]
        ...
        .map(|c| c.name.as_str())
        .collect();
    assert_eq!(jan, vec!["2026-01-02-a", "2026-01-15-b"], "组内按目录名稳定排序");
}
...
fn created优先取workflow_json_archive无created时回退目录名前缀() {
    ...
    let archived = ws.0.join("openspec/changes/archive/2026-07-06-backtrack-reason-propagation");
    ...
    assert_eq!(summary.created.as_deref(), Some("2026-07-06"), "回退目录名日期前缀");
    ...
    assert_eq!(list2.archive_groups[0].changes[0].created.as_deref(), Some("2026-01-01"));
}
...
fn workflow_json不可解析的change标注unparsable且仍入列() {
    ...
    assert_eq!(list.active[0].inventory, Inventory::V2, "文件存在按现役代际假设");
}
...
fn 单月单元素分组正确成组() {
    ...
    ws.change("openspec/changes/archive/2025-11-09-only-one", &[("proposal.md", "# v0")]);
