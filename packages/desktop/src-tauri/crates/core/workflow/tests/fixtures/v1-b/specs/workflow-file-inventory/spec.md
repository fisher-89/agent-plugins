...
`workflow.json` SHALL 新增可选字段 `files`，形状为 `{ written: string[], deleted: string[] }`——两个路径列表，路径为相对项目根的 POSIX 风格路径（条目可选携带来源审计字段 `agent_type`，见「清单条目来源审计」要求；承载形态——flat 字符串数组＋旁挂来源映射或条目内嵌对象——由 design 定，消费方契约仅依赖路径）；目录删除 SHALL 以目录路径本身记录，消费方按路径前缀匹配。`written` 桶内 SHALL NOT 再按 add/modify 细分（该区分由 design 变更清单承载）。
...
插件 SHALL 注册 PostToolUse hook 记录器，覆盖工具 `Write`、`Edit`、`NotebookEdit`（取 `tool_input.file_path` / `tool_input.notebook_path`）与 `Bash`、`PowerShell`（经 `extractFileOps` 提取）。记录器 SHALL 将每次文件操作折叠为净状态后写入当前 change 的 `workflow.json` 的 `files` 字段，条目来源按「清单条目来源审计」要求随事件 `agent_type` 落盘。
...
hook 认不出的还原路径 SHALL 走 `change_files` 工具的 `set` 语义显式修正；批量还原（`git stash` / `git clean`）由 PreToolUse 拦截（见 protect-files-hook 变更），phase 内增量还原以 `git restore` 为规范动作（见 phase-agents 变更）。
...
### Requirement: backtrack 清空清单重记

`backtrack` MCP 工具 SHALL 在回溯目标为 `implement` phase 或其之前的 phase（含 `proposal`、`dev-design`、`test-design`）时，清空该 change 的 `files`（重置为 `{ written: [], deleted: [] }`）以重记；回溯到 `implement` 之后的 phase（如 `test-gen`、`test-execution`）MUST NOT 清空清单。

#### Scenario: 回溯到 implement 清空

- **WHEN** 对 change 调用 `backtrack(phase: "test-execution", backtrack_to: "implement", ...)`
- **THEN** 该 change 的 `files` 被重置为空净状态
- **AND** 此后新的文件操作重新折叠记录

#### Scenario: 回溯到 test-gen 不清空

- **WHEN** 对 change 调用 `backtrack(backtrack_to: "test-gen", ...)`
- **THEN** `files` 保持不变

## Module Contract

### Schema: `plugins/dev-team/bin/src/schemas/workflow.schema.ts`

| 字段    | 类型                                               | 说明                                                                                                                      |
| ------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `files` | `{ written: string[], deleted: string[] }`（可选） | 文件清单净状态；`change_create` 初始化；消费方对缺失硬报错；条目可选携带来源 `agent_type`（形态 design 定，消费方不依赖） |

### Module: `plugins/dev-team/bin/src/hooks.ts`（新增子命令）

| 符号                     | 变更                                                                                         |
| ------------------------ | -------------------------------------------------------------------------------------------- |
| `extractFileOps`         | **ADDED** 共享三态提取器（泛化 `extractBashWriteTargets` / `extractPowerShellWriteTargets`） |
| PostToolUse 记录器子命令 | **ADDED** 读 stdin 事件 → 查 session 注册表 → 折叠 → 过滤 → 写 `workflow.json.files`         |
| 净状态折叠               | **ADDED** write/delete/revert 三条对称规则                                                   |
| 来源审计                 | **ADDED** 条目随事件 stdin `agent_type` 落盘（主会话/手动补录省略；同路径重写取最后写入者）  |

### Module: session 注册表（新增，如 `bin/src/lib/session-registry.ts`）

| Property     | Description                                                                     |
| ------------ | ------------------------------------------------------------------------------- |
| **Key**      | `session_id`（Claude Code stdin 唯一会话通道，无 `CLAUDE_SESSION_ID` 环境变量） |
| **Value**    | change 名称                                                                     |
| **建立时机** | PostToolUse 观察到全名 `mcp__plugin_dev-team_dev-team__phase_next` 调用事件     |
| **持久化**   | 落盘注册表（具体位置 design 定）                                                |

### Module: `plugins/dev-team/bin/src/lib/c4-cross-ref.ts`

| 符号              | 变更                                                                                                                                     |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `getChangedFiles` | **MODIFIED** 清单模式数据源 `git diff --cached` → `workflow.json.files.written`；既有 `files` 显式入参优先级不变；模型文件不进被查文件集 |

### Module: `plugins/dev-team/bin/src/commands/backtrack.ts`

| 符号     | 变更                                                       |
| -------- | ---------------------------------------------------------- |
| 清空规则 | **ADDED** `backtrack_to` 索引 ≤ `implement` 时重置 `files` |

### Hooks 注册: `plugins/dev-team/hooks/hooks.canonical.json`

| Property                           | Description                                                                                                                                               |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **postToolUse**                    | **ADDED** matchers：`Write\|Edit\|NotebookEdit\|Bash\|PowerShell` 与全名 `mcp__plugin_dev-team_dev-team__phase_next`，command 指向 hooks bin 记录器子命令 |
| **既有 preToolUse / subagentStop** | 不变（保护扩拦截见 protect-files-hook 变更）                                                                                                              |

### 移除: `plugins/dev-team/bin/src/lib/git.ts`

| 符号              | 变更                                                                          |
| ----------------- | ----------------------------------------------------------------------------- |
| `getGitDiffFiles` | **REMOVED**（连同 `git add -N` / `git reset` index 改写与 `simple-git` 依赖） |
