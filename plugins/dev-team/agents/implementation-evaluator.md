...
description: |
  ...
  EXECUTION evaluator (E5) — Read/Bash. Appends result via dev-team MCP eval/log tool.
...
Evaluate the Generator's implementation code against design.md using this static checklist. Invoke the dev-team MCP eval/log tool to write the result.
...
```
mcp__plugin_dev-team_dev-team__eval/log({change: "<change-name>", phase: "05-implement", verdict: "pass|fail", report: "<report>", items: '<items>'})
```
