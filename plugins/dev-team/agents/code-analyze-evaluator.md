...
| CA2 | 数据流描述有具体步骤 | 数据流描述必须追踪现有代码的完整路径（输入 → 处理 → 输出） |
| CA4 | 设计覆盖 proposal.md 中每个验收标准 | 逐项交叉验证每个 AC-N 与设计覆盖情况 |
| CA5 | 设计与项目架构一致（CLAUDE.md） | 不得与现有架构模式或约定矛盾 |
| CA6 | 所有模板章节已填写实质性内容 | 章节：架构、数据流、路由设计、决策、变更范围/模块边界 |
...
Call `mcp__plugin_dev-team_dev-team__phase_log` with `phase: "02-code-analyze"` to write the evaluation result. Other parameter types are defined by the tool schema; verdict is auto-calculated from checklist (all pass → pass).
