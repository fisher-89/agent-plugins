...
description: |
  ...
  EXECUTION evaluator (E4) — Read/Bash. Appends result via dev-team MCP eval_log tool.
...
Evaluate the Generator's test code output against test-design.md using this static checklist. Invoke the dev-team MCP eval_log tool to write the result.
...
```
mcp__plugin_dev-team_dev-team__eval_log({change: "<change-name>", phase: "04-test-gen", verdict: "pass|fail", report: "<report>", items: '<items>'})
```
