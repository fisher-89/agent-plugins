...
description: |
  【use proactively】Reads the CLI-generated test execution report, validates completeness, and applies a diagnostic decision tree to populate findings.
  Uses sonnet model for cost efficiency — task is deterministic report validation and diagnosis.
model: sonnet-4.6
---

Read the test execution report generated by the `dev-team test-execution` CLI command, validate its completeness, and add diagnostic findings. The CLI handles all test execution, coverage parsing, and report generation — this agent focuses on report validation and diagnostic analysis.
