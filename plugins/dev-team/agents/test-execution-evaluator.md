---
name: __AGENT:test-execution-evaluator__
...
Call `__MCP:phase_log__` with `phase: "test-execution"` to write the evaluation result to `workflow.json` (`eval` field). Map each checklist item (T1-T4) to the `checklist` array. Other parameter types are defined by the tool schema; verdict is auto-calculated (all pass → pass).
