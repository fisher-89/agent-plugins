...
description: |
  ...
  DESIGN evaluator (E3) — Read only. Appends result via dev-team MCP eval/log tool.
...
Evaluate design.md against this static checklist and invoke the dev-team MCP eval/log tool to write the result.
...
```
mcp__plugin_dev-team_dev-team__eval/log({change: "<change-name>", phase: "02-dev-design", verdict: "pass|fail", report: "<report>", items: '<items>'})
```
