...
7. **集成测试框架识别**：调用 `mcp__plugin_dev-team_dev-team__test_detect_frameworks`，传入步骤 6 汇总的模块文件列表，识别项目使用的测试框架和测试区域。根据返回的框架信息（如 vitest、jest、mocha 等）确定集成测试文件的扩展名、断言库和测试运行器
8. 从 proposal.md / design.md 识别需要集成测试覆盖的场景。对每个场景，在受测模块最近的 `__tests__/<scenario>/` 目录下创建集成测试用例）。集成测试文件扩展名和测试框架应与步骤 7 的 `test_detect_frameworks` 结果一致
9. **单元测试路径**：调用 `mcp__plugin_dev-team_dev-team__test_resolve_paths`，传入 `modules` 参数获取单元测试路径
