...
- The change scope comes from the design/proposal declaration × the change file inventory fetched via `__MCP:workflow_files__` × the filesystem
...
3. Call `__MCP:workflow_files__({ change: "<change-name>" })` 获取 `files` 清单圈定审查范围；run `git diff` to view the modification content
