...
- Skip the phase: append a skipped entry to eval.json via MCP:
  ```
  mcp__plugin_dev-team_dev-team__phase_log({change: "<name>", phase: "08-integration-test", report: "No integration tests found, phase skipped (no-op)", items: '[]', backtrack_to: null, skipped: true, findings: "未发现集成测试文件，阶段跳过"})
  ```
- Phase complete.
