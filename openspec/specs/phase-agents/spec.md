...
此外，本变更还更新以下内容：
- 评估器的静态检核表格式从四列（ID、检查项、必须、证据提示）变为三列（ID、检查项、证据提示），不再包含 `必须` 列。
...
| proposal-evaluator | Read, phase/log | Same checklist as requirements-evaluator; phase `01-proposal`; 3-column checklist (ID/检查项/判断依据); verdict: ALL items pass |
| architecture | archi/query, archi/validate, archi/write, archi/check, phase/log | All references use `xx/yy` format |
| dev-design-evaluator | Read, phase/log | Phase `02-dev-design`; 3-column checklist; D7/D9 mandatory; verdict: ALL items pass |
| test-design-evaluator | Read, phase/log | Phase `03-test-design`; 3-column checklist; T5/T8 mandatory; verdict: ALL items pass |
| test-gen-evaluator | Read, phase/log | 3-column checklist; verdict: ALL items pass |
| implementation-evaluator | Read, phase/log | 3-column checklist; verdict: ALL items pass |
| code-review-evaluator | Read, phase/log | 3-column checklist; verdict: ALL items pass (C1-C8) |
| acceptance-evaluator | Read, phase/log | 3-column checklist; A5 mandatory; verdict: ALL items pass (A1-A5) |
| unit-test-evaluator | Read, phase/log | 决策树格式（无静态检核表）；不涉及 checklist 格式变更 |
| integration-test-evaluator | Read, phase/log | 决策树格式（无静态检核表）；不涉及 checklist 格式变更 |
