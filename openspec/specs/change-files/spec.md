...
提供 `change_files` MCP 工具，作为 change 文件清单（`workflow.json` 的 `file_log` 字段，净状态经派生视图读取）的手动补录与显式修正通道：`append` 为 workflow scope 按 path upsert，`set` 为净状态覆写。
...
`plugins/dev-team/bin/src/mcp.ts` SHALL 注册名为 `workflow_files` 的 MCP 工具，作为 change 文件清单（`workflow.json` 的 `file_log` 字段，净状态经派生视图读取）的只读查询通道，供 agent 代替直接 Read `workflow.json`。工具 SHALL 接收以下输入：
...
- **THEN** workflow scope upsert / 覆写语义、phase 审计保留策略与 `workflow_type` / `created` / `eval` / `active_phase` 保留行为与规格一致
