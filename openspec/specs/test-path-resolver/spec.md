...
- `modules` 类型从 `string[]` 扩展为 `string[] | "git-change"`（`"git-change"` 字面量由清单模式字面量替代，建议 `"change"`，最终命名 design 定）
...
- 清单读取失败（替代原 git diff 失败：目标 change 不存在、`workflow.json` 缺失或非法、`files` 字段缺失等，仅清单模式）
...
当 `modules` 为清单模式字面量（替代原 `"git-change"`）时，函数 SHALL：
...
| **Input** | Command 层可含 `project_root`（CLI/单测 fixture）；与 MCP 必填契约一致时可复用同一字段 |
...
| **Input schema** | `{ modules: z.union([z.array(z.string()), z.literal("git-change")]), project_root: z.string() }`（`project_root` 必填） |
...
| **Behavior** | Zod v4 schema；`modules` 为 `string[] | "git-change"`，可为空数组；`project_root` 必填；与 MCP 注册一致 |
