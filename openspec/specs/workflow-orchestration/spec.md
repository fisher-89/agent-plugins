...
#### Scenario: phase_next resumes from partial completion
- **WHEN** `phase_next` is called for a change that has pass entries for phases 01-03 but no entries for phase 04 onward
...
| Tool | Change | Purpose |
|------|--------|---------|
| phase_next | MODIFIED | Removes `clearEntriesFromPhase()`; uses `hasPhasePassed()` with stale filtering; no longer modifies eval.json |
| phase_log | MODIFIED | When writing entry with `backtrack_to`: calls `markPhaseStale()` for each target (which internally propagates). Pass entries require no extra action. |
