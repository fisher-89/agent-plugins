...
### Requirement: config_set writes key values to config.json

The system SHALL provide a `config_set` MCP tool (formerly `config/set`) that writes values to `openspec/config.json`.
...
- **WHEN** the user calls `config_set` with `{"key": "static_check", "value": ["eslint src/", "tsc --noEmit"]}`
...
- **WHEN** the user calls `config_set` with `{"key": "test_scripts.unit", "value": "npm test"}`
...
- **WHEN** the user calls `config_set` with `{"key": "schema", "value": "spec-driven"}`
...
- **WHEN** the user calls `config_set` with `{"key": "static_check", "value": ["eslint src/"]}`
...
- **WHEN** the user calls `config_set` with `{"key": "schema", "value": null}`
