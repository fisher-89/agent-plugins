#!/usr/bin/env bash
# setup_test_env.sh — Shared test helpers for add-dev-team-cli-eval-log tests.
#
# Provides:
...
#   setup_eval_log_fixture()   — Copy a fixture eval.json into the sandbox change
...
#   assert_json_array_length() — Assert a JSON array file has expected length
#
# Usage:
...
setup_sandbox() {
    SANDBOX_DIR="$(mktemp -d -t eval_log_test_XXXXXX)"
    export SANDBOX_DIR
    ...
    export TEST_CHANGE="test-change"
...
setup_dev_team_cli() {
    ...
    local project_root
    project_root="$(cd "$(dirname "$0")/../../../../../plugins/dev-team/bin" && pwd 2>/dev/null || true)"
    ...
    if [[ -f "$project_root/dev-team-bundle.js" ]]; then
        ...
        # Try alternate project layout (from tests directory perspective)
        project_root="$(cd "$(dirname "$0")/../../../../plugins/dev-team/bin" && pwd 2>/dev/null || true)"
        if [[ -f "$project_root/dev-team-bundle.js" ]]; then
            bundle_path="$project_root/dev-team-bundle.js"
        else
...
setup_eval_log_fixture() {
    local fixture_name="$1"

    local fixtures_dir
    fixtures_dir="$(cd "$(dirname "$0")/../fixtures" && pwd 2>/dev/null || true)"
    ...
    cp "$fixture_file" "$TEST_PHASES_DIR/eval.json"
    echo "PASS: Loaded fixture '$fixture_name' into $TEST_PHASES_DIR/eval.json"
}
...
assert_json_array_field() {
    ...
    local actual
    actual="$(python3 -c "import json; d=json.load(open('$file')); print(d[$index].get('$field', '<<MISSING>>'))" 2>/dev/null || echo '<<PARSE_ERROR>>')"

    if [[ "$actual" == "$expected" ]]; then
        echo "PASS: JSON array[$index] field '$field' = '$expected'"
    else
        echo "FAIL: JSON array[$index] field '$field' expected '$expected', got '$actual'" >&2
        exit 1
    fi
}
...
assert_json_array_length() {
    ...
    local actual
    actual="$(python3 -c "import json; d=json.load(open('$file')); print(len(d))" 2>/dev/null || echo '<<PARSE_ERROR>>')"

    if [[ "$actual" == "$expected" ]]; then
        echo "PASS: JSON array length = $expected"
    else
        echo "FAIL: JSON array length expected $expected, got $actual" >&2
        exit 1
    fi
}

# ─── Phase verification helper ───────────────────────────────────────────────

assert_phase_verdict() {
    local file="$1"
    local phase="$2"
...
verdicts = [e.get('verdict') for e in data if e.get('phase') == '$phase']
print(','.join(verdicts) if verdicts else 'NO_ENTRIES')
" 2>/dev/null || echo '<<PARSE_ERROR>>')"

    if [[ "$actual" == *"$expected_verdict"* ]]; then
        echo "PASS: Phase '$phase' has verdict '$expected_verdict'"
    else
        echo "FAIL: Phase '$phase' expected verdict '$expected_verdict', got '$actual'" >&2
        exit 1
    fi
}
