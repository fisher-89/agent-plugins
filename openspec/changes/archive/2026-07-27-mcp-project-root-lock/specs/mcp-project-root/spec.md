...
**Reason**: MCP 路径禁止静默回退 `process.cwd()` / 不可靠的多路径 `[0]`；启动锁定改用「唯一可信源」严格优先级（`CLAUDE_PROJECT_DIR` → 恰好 1 个 roots → 恰好 1 个 `WORKSPACE_FOLDER_PATHS`）。CLI / 非 MCP 仍可通过 command `options.projectRoot` 或各自入口解析。

**Migration**: 用 ADDED「Startup lock resolves a unique project root」替换；删除对 `CURSOR_PROJECT_DIR` 与 cwd 末位回退的 MCP 依赖。测试改为注入绝对路径 env 或锁定缓存，不再断言 cwd 回退。

### Requirement: Project root cache refreshes on roots/list_changed notification

**Reason**: Cursor / Claude Code 几乎不推送换根；与「启动锁定、进程内不可变」目标冲突；现有代码亦未真正注册 notification handler。

**Migration**: 不注册 `notifications/roots/list_changed`；换工作区依赖宿主重启 MCP 子进程。删除相关刷新场景测试，改为断言二次 init 不改写已锁定缓存。

### Requirement: Explicit project_root tool parameter overrides MCP resolution

**Reason**: 模型不得 override 宿主绑定根；MCP 对外契约删除 `project_root` input。

**Migration**: 从各 MCP Zod/input schema 删除该字段；handler 一律走锁定根。CLI / command 层 `options.projectRoot` 保留供单测与 CLI fixture，但不属于 MCP tool API。
