 ...
 * @see openspec/changes/add-config-mcp-tools/specs/config-set/spec.md
 */
...
import { readConfig, writeConfig } from '../../../../../plugins/dev-team/bin/src/lib/config';
...
describe('runConfigSet — command logic (unit)', () => {
  ...
  it('should preserve value types as-is (no heuristic type coercion)', () => {
    runConfigSet({ key: 'flag', value: 'true', projectRoot: tmpDir });
    const config = readConfig(tmpDir);
    // Value is stored as-is — "true" remains string "true" (no boolean coercion)
    expect(config.flag).toBe('true');
    // Number values also pass through unchanged
    runConfigSet({ key: 'count', value: 42, projectRoot: tmpDir });
    const config2 = readConfig(tmpDir);
    expect(config2.count).toBe(42);
  });
...
describe('Integration: config/set filesystem behavior', () => {
  ...
  it('should auto-create openspec/config.json when writing to empty directory', () => {
    ...
    expect(parsed).toEqual({ schema: 'spec-driven', context: 'test' });
  });
...
describe('config/set idempotency', () => {
  ...
  it('should correctly overwrite when setting same key with different value (幂等性-2)', () => {
    runConfigSet({ key: 'schema', value: 'v1', projectRoot: tmpDir });
    runConfigSet({ key: 'schema', value: 'v2', projectRoot: tmpDir });
    const result = readConfig(tmpDir);
    expect(result.schema).toBe('v2');
    expect(Array.isArray(result.schema)).toBe(false);
  });
});
