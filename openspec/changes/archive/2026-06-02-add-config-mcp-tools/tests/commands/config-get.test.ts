 ...
 * @see openspec/changes/add-config-mcp-tools/specs/config-get/spec.md
 */
...
describe('runConfigGet — command logic', () => {
  ...
  it('should return value and exists: true for an existing key', () => {
    // runConfigGet calls ensureConfigFile which creates skeleton with schema: spec-driven
  ...
  it('should pass project_root to readConfigValue and use a custom root', () => {
    ...
    try {
      ...
      // ensureConfigFile creates skeleton with schema: spec-driven
      expect(result.exists).toBe(true);
      expect(result.value).toBe('spec-driven');
    } finally {
  ...
  it('should auto-create skeleton when project_root does not yet exist', () => {
    ...
    try {
      ...
      expect(result.exists).toBe(true);
      expect(result.value).toBe('spec-driven');
      expect(fs.existsSync(path.join(nonExistent, 'openspec', 'config.json'))).toBe(true);
    } finally {
