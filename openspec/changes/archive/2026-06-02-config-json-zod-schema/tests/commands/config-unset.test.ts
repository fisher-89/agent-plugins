...
describe('runConfigUnset — command logic (unit)', () => {
  ...
  it('should forward project_root parameter', () => {
    ...
    try {
      // First write a key (context is optional, so unset is observable)
      runConfigSet({ key: 'context', value: 'test-context', projectRoot: customDir });
      const result = runConfigUnset({ key: 'context', projectRoot: customDir });
      expect(result.removed).toBe(true);
  ...
  it('should preserve other keys when removing one key', () => {
    ...
    runConfigSet({ key: 'context', value: 'keep-me', projectRoot: tmpDir });
    const result = runConfigUnset({ key: 'static_check', projectRoot: tmpDir });
    expect(result.removed).toBe(true);
    const config = readConfig(tmpDir);
    // static_check should be removed
    expect((config as Record<string, unknown>).static_check).toBeUndefined();
