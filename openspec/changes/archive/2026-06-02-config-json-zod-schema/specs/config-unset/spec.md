...
The system SHALL provide a `config/unset` MCP tool that removes keys from `openspec/config.json`.
...
This requirement is unchanged from the YAML-based implementation.
...
| **Signature** | `unsetConfigValue(projectRoot: string, key: string): { removed: boolean }` |
| **Input** | `projectRoot` — project root directory path; `key` — dot-separated key path |
| **Output** | `{ removed: boolean }` — whether a value was actually removed |
| **Behavior** | Reads `config.json`, removes value at key path, serializes back with `JSON.stringify`. Returns `removed: false` if key or file does not exist |
