...
**Reason**: Config storage format has changed from YAML to JSON. The `yaml` npm package is no longer needed in the `config/set` write path. JSON serialization uses native `JSON.stringify`/`JSON.parse`, which does not require any external library. Zod schema validation (`parseConfig`) replaces YAML document manipulation and comment preservation for ensuring structure correctness.

**Migration**: No caller migration needed — the `config/set` tool input/output schema is unchanged. The `yaml` library remains in `package.json` dependencies for the YAML-to-JSON auto-migration path in `readConfig()`, but the `config/set` write path uses native JSON exclusively.
