...
The default denial reason SHALL follow the format:
```
"File '%s' is write-protected. 检测到对受保护文件 '%s' 的直接写入（通过 %s 工具）。请使用专有工具（phase_log / config_get / config_set）替代。直接写入将破坏数据完整性。"
```

Where the first `%s` is the file path, second `%s` is the file path, and `%t` is the tool name. Wait — the format uses positional: `%s` = change name (for eval.json) or file path (for config.json), `%t` = tool name.
...
The hook SHALL iterate over `write_protection.files` and match the target file path against each entry's `glob` pattern using the `minimatch` library (or equivalent glob matching).
...
| **Glob library** | `minimatch` (or equivalent) |
