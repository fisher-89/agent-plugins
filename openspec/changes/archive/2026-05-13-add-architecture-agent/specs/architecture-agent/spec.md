...
The architecture agent SHALL support a propose mode that reads current model files and relevant code, drafts DSL changes, validates them via `archi-model.py validate --source`, and presents the diff to the user for confirmation.

#### Scenario: Propose a new component

- **WHEN** the user asks to add a component for `src/services/payment/`
- **THEN** the agent SHALL read existing `models/*.c4`, grep for payment-related code, draft a DSL snippet, validate it, and present the proposal with a diff

#### Scenario: Proposal validation fails

- **WHEN** the drafted DSL snippet fails `archi-model.py validate --source`
