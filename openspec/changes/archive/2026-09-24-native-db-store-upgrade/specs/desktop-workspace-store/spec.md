...
### RENAMED: redb 类型不泄漏与 schema_version → native_db 类型不泄漏与模型版本治理

- **FROM**: `### Requirement: redb 类型不泄漏与 schema_version` —— store 公共 API 只暴露自有类型；redb 类型不出现在公共签名；META 表自第一天记录 `schema_version`
- **TO**: `### Requirement: native_db 类型不泄漏与模型版本治理` —— 内容如下：
...
| 模块 | 职责 | 关键契约 |
|------|------|----------|
| `packages/desktop/src-tauri/crates/infra/store`（crate 名 `store`） | native_db 本地库，db 边界第一成员 | `Store::open(path)` 注入式打开 + legacy 探测迁移；workspace 三操作 + 信封 API；公共 API 仅自有类型；零 Tauri；仅依赖 `agent` 纯类型作嵌装载荷；迁移期保留 redb（迁移模块内部） |
