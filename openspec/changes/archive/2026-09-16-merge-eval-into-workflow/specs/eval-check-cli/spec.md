...
编排门禁与评估写入的 MCP 契约。phase 集合见 `pge-workflow-engine`；回溯写入见 `pipeline-backtrack`。`phase_check` 仍退役。本变更只改 `phase_log` 的持久化文件。
...
phase 序列以 `getPhaseTable(workflow_type)` 为准。
