...
| 框架版本探测 | bin/__tests__/test-setup.ts 全局 mock（既有 setup） | 全部场景 |
...
| Mock主体 | Mock方案 | 应用场景 |
|----------|----------|----------|
| ProductEnv | 构造 claude / cursor 两个 env fixture（沿用既有 build/__tests__/hooks-platform-subagent-stop 的 env 构造方式） | 全部场景 |
