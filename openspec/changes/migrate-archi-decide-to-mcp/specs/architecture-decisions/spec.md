...
### Function: createAdr / listAdrs / updateAdr

| Property | Description |
|----------|-------------|
| **Module** | `plugins/dev-team/bin/src/lib/archi-decide.ts` |
| **Signatures** | `createAdr(projectRoot, input): CreateResult`; `listAdrs(projectRoot, status?): ListResult`; `updateAdr(projectRoot, input): UpdateResult` |
| **Input** | create: title, background, decision, optional consequences/alternatives/scope/status; list: optional status; update: file, status, optional superseded_by |
| **Output** | JSON-serializable success/error structures compatible with MCP structuredContent |
| **Behavior** | Files under `<projectRoot>/openspec/architecture/decisions/`; create renders `templates/adr.md`; statuses limited to `proposed` \| `accepted` \| `deprecated` \| `superseded`; `superseded` requires `superseded_by` |

### API: archi_decide (MCP tool)

| Property | Description |
|----------|-------------|
| **Tool name** | `archi_decide` |
| **Input schema** | `{ project_root: string, action: 'create'\|'list'\|'update', ...action-specific fields }` |
| **Output schema** | Discriminated by action; always JSON via `jsonContent` |
| **Registration** | `plugins/dev-team/bin/src/mcp.ts` — handler wrapped in `withResolvedProjectRoot` |
| **Schema module** | `plugins/dev-team/bin/src/schemas/archi-decide.schema.ts` |
