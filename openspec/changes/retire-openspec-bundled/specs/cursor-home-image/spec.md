...
### Requirement: Home image product tree is committed and installable (UNCHANGED)
...
#### Scenario: Home image directory exists after build (UNCHANGED)
...
#### Scenario: Home image is not gitignored (UNCHANGED)
...
#### Scenario: Home image version matches source package (UNCHANGED)
...
### Requirement: Home image layout mirrors plugin root under ~/.cursor (UNCHANGED)
...
#### Scenario: Sibling layout in the image (UNCHANGED)
...
#### Scenario: Prefixed skill and agent names (UNCHANGED)
...
### Requirement: Path tokens remain until install (UNCHANGED)
...
#### Scenario: Image retains path tokens (UNCHANGED)
...
#### Scenario: Install expands path tokens to absolute paths (UNCHANGED)
...
#### Scenario: Managed MCP merge preserves user servers (UNCHANGED)
...
#### Scenario: Managed hooks merge preserves user entries (UNCHANGED)
...
#### Scenario: Installer refuses blind overwrite (UNCHANGED)
...
#### Scenario: Non-JSON path separator normalization (UNCHANGED)
...
### Requirement: cursorHome user-level reference prefixes (UNCHANGED)
...
#### Scenario: No server-level MCP reference in source (UNCHANGED)
...
### Product: `cursor-home-image/dev-team/` (UNCHANGED)
