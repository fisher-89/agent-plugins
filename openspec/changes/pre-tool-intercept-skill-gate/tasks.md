...
- [ ] 1.1 Create `plugins/dev-team/utils/routing-state.py` with dataclass `PendingState` containing version, decision, intent, and options fields (no timestamp — turn-scope expiry)
...
- [ ] 1.5 Add unit tests for routing-state module functions

## 2. UserPromptSubmit Hook Enhancement

- [ ] 2.1 Add import for `write_pending_state` and `clear_pending_state` in `plugins/dev-team/hooks/on-user-prompt.py`
- [ ] 2.2 Add `clear_pending_state(cwd)` call at start of main() to clear stale state from previous turns (turn-scope expiry)
- [ ] 2.3 Add logic to write pending state when `decision.mode in ("auto", "suggest") and decision.action != "direct"`
- [ ] 2.4 Add skip workflow keyword detection ("implement directly", "skip", "直接实现") in IntentClassifier — when detected, skip writing pending state
...
- [ ] 3.2 Implement state reading at hook start — no expiration check needed (turn-scope expiry handled by UserPromptSubmit)
...
- [ ] 3.6 Return `deny` with additionalContext when pending state exists
- [ ] 3.7 Return `allow` when no pending state
...
- [ ] 6.4 Test: UserPromptSubmit clears old pending state at start of each turn (turn-scope expiry)
- [ ] 6.5 Test: PreToolUse Skill hook clears state when openspec skill is invoked
- [ ] 6.6 Test: UserPromptSubmit skips writing pending state when user types skip keywords
